...
namespace {
    ...
    float getTDCTimeFromSample(const QIE11DataFrame::Sample& s)
    {
        ...
        if (tdc == qie11_tdc_code_overshoot)
            t = HBHEChannelInfo::UNKNOWN_T_OVERSHOOT;
        else if (tdc == qie11_tdc_code_undershoot)
            t = HBHEChannelInfo::UNKNOWN_T_UNDERSHOOT;
        return t;
    }

    float getTDCTimeFromSample(const HcalQIESample&)
    {
        return HBHEChannelInfo::UNKNOWN_T_NOTDC;
    }

    // The first element of the pair indicates presence of
