...
void HBHEPhase1Reconstructor::processData(const Collection& coll,
  ...
  for (typename Collection::const_iterator it = coll.begin(); it != coll.end(); ++it) {
    ...
    // Look up channel properties
    const HBHEChannelProperties& properties(channelProperties_.at(htopo.detId2denseId(cell)));

    // Make sure these properties were not created by the default constructor
    assert(properties.channelCoder);
...
void HBHEPhase1Reconstructor::produce(edm::Event& e, const edm::EventSetup& eventSetup) {
  ...
  if (processQIE8_) {
    ...
    processData<HBHEDataFrame>(
        *hbDigis, *conditions, isData, &channelInfo, infos.get(), out.get(), use8ts_);
    if (setNoiseFlagsQIE8_)
  ...
  if (processQIE11_) {
    ...
    processData<QIE11DataFrame>(
        *heDigis, *conditions, isData, &channelInfo, infos.get(), out.get(), use8ts_);
    if (setNoiseFlagsQIE11_)
...
void HBHEPhase1Reconstructor::rebuildChannelProperties(edm::EventSetup const& eventSetup) {
  using namespace edm;

  // Fetch various calibrations
...
void HBHEPhase1Reconstructor::beginRun(edm::Run const& r, edm::EventSetup const& es) {
  ...
  // Rebuild the channel properties table
  rebuildChannelProperties(es);
}
